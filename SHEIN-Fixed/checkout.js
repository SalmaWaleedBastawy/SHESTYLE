...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function loadRows() {
    ...
    cartItems.forEach(function (item) {
      ...
      for (var i = 0; i < catalog.length; i++) {
        if (catalog[i].id === item.id) { product = catalog[i]; break; }
      }
      if (product) out.push({ item: item, product: product });
    });
    return out;
  }
  ...
  function renderSummary() {
    ...
    if (summaryShipping) summaryShipping.textContent = shipping === 0 ? 'FREE' : formatMoney(shipping);
    if (summaryTotal) summaryTotal.textContent = formatMoney(total);
  }

  /* ---------------- Step navigation ---------------- */

  var stepLabels = { 1: 'CONTINUE TO PAYMENT', 2: 'CONTINUE TO REVIEW', 3: 'PLACE ORDER' };
  ...
  function showStep(step) {
    ...
    Object.keys(panels).forEach(function (key) {
      panels[key].style.display = (Number(key) === step) ? '' : 'none';
    });
    ...
    if (primaryBtn) primaryBtn.textContent = stepLabels[step] || 'CONTINUE';
  ...
  stepButtons.forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      if (target <= maxStepReached && target < currentStep) showStep(target);
    });
  });

  checkoutForm.querySelectorAll('.checkout-back-btn, .review-edit').forEach(function (btn) {
    btn.addEventListener('click', function () { showStep(Number(btn.dataset.backTo)); });
  });
  ...
  function validateShipping() {
    ...
    var ok = true;
    ok = setFieldValid(fullName, fullName.value.trim().length > 1) && ok;
    ok = setFieldValid(email, emailOk) && ok;
    ok = setFieldValid(address, address.value.trim().length > 3) && ok;
    ok = setFieldValid(city, city.value.trim().length > 1) && ok;
    ok = setFieldValid(postalCode, postalCode.value.trim().length > 1) && ok;
    ok = setFieldValid(phone, phoneDigits.length >= 7) && ok;

    if (ok) {
      shippingData = {
        fullName: fullName.value.trim(), email: email.value.trim(), address: address.value.trim(),
        city: city.value.trim(), postalCode: postalCode.value.trim(), phone: phone.value.trim()
      };
    }
    return ok;
  }
  ...
  function validatePayment() {
    ...
    var expiryMatch = /^(\d{2})\/(\d{2})$/.exec(cardExpiry.value.trim());
    var expiryOk = false;
    if (expiryMatch) {
      var month = parseInt(expiryMatch[1], 10);
      var year = parseInt(expiryMatch[2], 10) + 2000;
      if (month >= 1 && month <= 12) {
        expiryOk = new Date(year, month) > new Date();
      }
    }

    var ok = true;
    ok = setFieldValid(cardName, cardName.value.trim().length > 1) && ok;
    ok = setFieldValid(cardNumber, digits.length === 16) && ok;
    ok = setFieldValid(cardExpiry, expiryOk) && ok;
    ok = setFieldValid(cardCvv, /^\d{3}$/.test(cardCvv.value.trim())) && ok;

    if (ok) {
      paymentData = { cardName: cardName.value.trim(), last4: digits.slice(-4), expiry: cardExpiry.value.trim() };
    }
    return ok;
  }

  function populateReview() {
    var reviewShipping = document.getElementById('reviewShipping');
    var reviewPayment  = document.getElementById('reviewPayment');
    ...
    if (reviewShipping && shippingData) {
      reviewShipping.innerHTML =
        ...
        escapeHtml(shippingData.address) + ', ' + escapeHtml(shippingData.city) + ' ' + escapeHtml(shippingData.postalCode) + '<br>' +
        escapeHtml(shippingData.email) + ' · ' + escapeHtml(shippingData.phone);
    }
    if (reviewPayment && paymentData) {
      reviewPayment.innerHTML =
        escapeHtml(paymentData.cardName) + '<br>Card ending in ' + escapeHtml(paymentData.last4) + ' · Expires ' + escapeHtml(paymentData.expiry);
    }
    if (reviewItems) {
  ...
  function placeOrder() {
    ...
    document.getElementById('confirmEmail').textContent = shippingData ? shippingData.email : '';
    document.getElementById('confirmTotal').textContent = formatMoney(totals.total);

    // Save a lightweight order record an admin dashboard could read later
    try {
      var orders = JSON.parse(localStorage.getItem('shestyle_orders') || '[]');
      orders.push({
        id: orderNumber,
        date: new Date().toISOString(),
        customer: shippingData ? shippingData.fullName : '',
        email: shippingData ? shippingData.email : '',
        total: totals.total,
        items: rows.map(function (r) { return { id: r.product.id, name: r.product.name, qty: r.item.qty, price: r.product.price }; }),
        status: 'Processing'
      });
      localStorage.setItem('shestyle_orders', JSON.stringify(orders));
    } catch (e) { /* ignore storage errors */ }

    if (typeof saveCart === 'function') saveCart([]); // clears the real cart + updates badges
  ...
  checkoutForm.addEventListener('submit', function (e) {
    e.preventDefault();
    if (currentStep === 1) { if (validateShipping()) showStep(2); return; }
    if (currentStep === 2) { if (validatePayment()) { populateReview(); showStep(3); } return; }
    if (currentStep === 3) { placeOrder(); }
  });
